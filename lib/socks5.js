...
function _handleSocksClient(client, remoteSocket, server, clientData, remoteData, closeServer) {
    ...
    if (remoteSocket) remoteSocket.on("timeout", _=>{remoteSocketError("timeout"); client.destroy();});
   
    setTimeout(function(){
    if (clientData) handleClientData(clientData); 
    if (remoteData) handleRemoteData(remoteData);
    if (closeServer) checkStopServer();
   },300)    // bad socks5 clients, send data to original port (libcurl!)
}
